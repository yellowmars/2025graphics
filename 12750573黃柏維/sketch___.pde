...
void draw() {
  background(255);
  camera();
  // 繪製背景圖片為一個大矩形貼圖，放在 3D 空間中
  pushMatrix();
  translate(0, 0, -300); // 往攝影機後方貼背景
  beginShape();
  texture(bg);
  noStroke();
  // 根據需要調整這個大小與位置
  vertex(-824, -467, 0, 0, 0);
  vertex(824, -467, 0, bg.width, 0);
  vertex(824, 467, 0, bg.width, bg.height);
  vertex(-824, 467, 0, 0, bg.height);
  endShape(CLOSE);
  popMatrix();
